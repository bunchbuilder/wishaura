...
(() => {
  ...
  // Safe localStorage wrapper — some mobile / in-app browsers (WhatsApp,
  // Instagram, Safari Private mode, some Android WebViews) throw on
  // localStorage access. An uncaught error here used to stop ALL the
  // code below it from ever running (loader stuck, form dead, photo
  // upload dead, preview dead) since it happened early in this file.
  const safeStorage = {
    get(key) {
  ...
  // (skipped on small/mobile screens for performance)
  (() => {
  ...
  const applyTheme = (theme) => {
    ...
    safeStorage.set("wa-theme", theme);
  };
  $$(".chip[data-theme], .theme-card[data-theme]").forEach((el) =>
    el.addEventListener("click", () => applyTheme(el.dataset.theme))
  );
  const savedTheme = safeStorage.get("wa-theme");
...
document.getElementById("downloadJpg").onclick = () => downloadCard("jpg");
  ...
  const savedMusic = safeStorage.get("wa-music") === "on";
  const setMusic = (state) => {
    ...
    safeStorage.set("wa-music", state ? "on" : "off");
  };
...
// Hide the loader. Guarded with a flag + try/catch so it can never run
// twice-badly or throw, and is safe to call from multiple triggers.
...
// Safety net: on some mobile networks/browsers a resource (fonts, the
// html-to-image CDN script, etc.) can stall or never fire "load". Never
// leave the user stuck staring at the loader — force it away after 5s.
